...
files_to_load = "employee_data1.csv"
files_to_output = "employee_data1.txt"
...
with open (file_to_load) as emp_data:
    ...
    for row in reader:
            ...
            reformatted_dob = datetime.datetime.strptime(row["DOB", ""])
            reformatted_dob = reformatted_dob.strftime("%m/%d/%Y")

            # Then store it into a list
            emp_dobs = emp_dobs + [reformatted_dobs]

            # Grab SSN and reformat it
            split_snn = list(row["SSN"])
            split_snn[0:3] = ("*","*","*")
            split_snn[4:6] = ("*","*")
            ...
            # Then store the abbreviations into a list
